<?php
	...
	*/
	...
	/*
		...
		2) 
			SELECT  * FROM stdscore	, no 필드를 기준으로 확인해보면 1, 2, 3, 4, 5... 순서로 작성되어 있음!
				: 최신의 글이 하단으로 작성됨!

			SELECT * FROM stdscore	ORDER BY no DESC 를 작성하면 no 필드를 기준으로 역순으로 정렬됨!
		...
		[정리]
			최신의 글을 가장 상단으로 가지고 오려면 , 필드의 정보를 역순으로 조회해서 가져와야 함!
	*/

	/*
		[sql문 작성하기]
			
			1) 요청할 sql문을 no 필드를 기준으로 역순으로 조회하기
			2) 로그인이 되어 있다면 sql문 요청하기!
	*/
	...
	/*
		result , 정보가 넘어가면 true 함께 생성됨!
			if($result == true)	, if($result)	,  result 값 자체가 true이기 때문에 생략도 가능함!
...
<?php }else{ ?>
			...
			<strong style="color:purple;"><?=$_SESSION['userid']?></strong>님 환영합니다.
